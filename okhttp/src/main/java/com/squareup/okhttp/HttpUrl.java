          ...
          if (c == '@') {
            ...
            try {
              while (!buffer.exhausted()) {
                ...
                if (p == '%'
                    && (buffer.size() < 2 || !isHex(buffer.getByte(0)) || !isHex(
                    buffer.getByte(1)))) {
                  throw new UnsupportedOperationException("TODO");
                  // TODO: what to do with this character? double-encode the `%` ?
                  // TODO: This case is broken. Likely a bug in the spec.
                } else if (p == ':' && result.password == null) {
                  result.password = new Buffer();
                } else {
                  if (result.password != null) {
                    utf8PercentEncode(result.password, c, DEFAULT_ENCODE_SET);
                  } else {
                    utf8PercentEncode(result.username, c, DEFAULT_ENCODE_SET);
                  }
                }
              }
